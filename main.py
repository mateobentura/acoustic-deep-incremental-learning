...
def main():
    ...
    BACKBONE = 'resnet34'
    var_time = timing()
    if sys.argv[1] == 'train': train(height, width, img_dir, sys.argv[2])
    elif sys.argv[1] == 'test':
        test = imsy.Image(300, noise_lvl=sys.argv[2])
        ...
        for noise_lvl in range:
            ...
            pad_h = 16
            pad_v = 16
            test.sliding_window(window_size, pad_h, pad_v, sys.argv[2])
            ds_test = ds.crops_to_dataset(test.crops, test.labels['classif'][:, :, 0], shuffle=False)
            test.calssification_predict(classif_model, ds_test, test.labels['classif'][:, :, 0].shape, sys.argv[2])
